...
class NutshellApi {
    ...
    public function call($method, array $params = null)
    {
        ...
        if (curl_errno($this->curl)) {
            throw new NutshellApiException(
                'Curl error #' . curl_errno($this->curl) . ' during API call: '. curl_error($this->curl)
            );
        }
        ...
        if ($fullResult->error !== null) {
            throw new NutshellApiException(
                'API Error: ' . $fullResult->error->message,
                $fullResult->error->code,
                $fullResult->error->data
            );
        }
    ...
    protected function _getApiEndpointForUser($username)
    {
        ...
        if (curl_errno($curl)) {
            throw new NutshellApiException(
                'Curl error #' . curl_errno($curl) . ' while finding endpoint: '. curl_error($curl)
            );
        }
        curl_close($curl);
